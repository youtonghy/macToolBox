...
8. **分应用音频** — macOS 14.2+ 使用公开 Core Audio Process Tap，为正在播放音频的应用设置 0%–300% 音量并选择输出设备；100% 为原始增益。超过 100% 可能削波失真。弹窗支持横向滚动的应用列表和竖向音量条；音量条默认显示 0%–100%，到达顶部后继续向上可依次扩展到 200% 和 300%。设置中可启用 Lite 模式，仅显示图标与百分比，并在聚焦后用滚轮或方向键调节。
