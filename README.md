...
- **外接显示器媒体键（亮度 / 音量）**：需要 **输入监控 (Input Monitoring)**。
  - 设置 → 通用 可查看授权状态、打开系统设置、重新检测。
  - 首次创建事件监听失败时会弹窗引导；也可手动到 *系统设置 → 隐私与安全性 → 输入监控* 打开 ToolBox。
  - **刚打开开关后若仍失败，请完全退出并重新打开 ToolBox**（macOS TCC 对当前进程常需重启才生效）。
